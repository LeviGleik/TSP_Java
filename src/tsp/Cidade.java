...
public class Cidade {
    ...
    public double[][] geraPopulacao(){
        
        int pos = (int)(Math.random()*100);
        int pos1 = (int)(Math.random()*100);
        int[] prevPos = new int[100];
        int[] prevPos1 = new int[100];
        double[][] populacao = new double[10][100];
        int k = 0;
        int t = 0;
        for(int i = 0; i < 10; i++){
            for(int j = 0; j < 100;){
                if(pos != pos1){
                    if(!contains(prevPos, pos)  && !contains(prevPos1, pos)){
                        prevPos[k] = pos;
                        k++;
                        if(!contains(prevPos1, pos1) && !contains(prevPos, pos1)){
                            prevPos1[k] = pos;
                            t++;
                            populacao[i][j] = this.xy[pos][pos1];
                            j++;
